...
class VBFblikelihood :
  ...
  def evaluateBlikelihood(self, event) :
    nJet=len(event.jets)#I havent found nJet variable, should I do like this?
    ...
    if ((event.jets[0].pt()>92.) and (event.jets[1].pt()>76.) and (event.jets[2].pt()>64) and (event.jets[3].pt()>30)):
      if (nJet>=4):
        jetsForHiggs4=[]
        jetsForHiggs4=[jet for index,jet in enumerate(event.jets) if (jet.jetID()>0) and (index<4) and (jet.pt()>20)]#jetID ,any arguments?
        ...
        if btag_max1>0.7:
          ...
          if (maxEta1>=0) and (maxEta2>=0) :
            ...
            if (Mqq>460) and(qqDeltaEta>4.1)and (bbDeltaPhi<1.6) :
              if (HLT_BIT_HLT_QuadPFJet_SingleBTagCSV_VBF_Mqq460_v==1) : #the name of the trigger variable

                loopMaxJet=7
                if nJet<7 : loopMaxJet=nJet  
                jetsForHiggsMax=[jet for index,jet in enumerate(event.jets) if (jet.id>0) and (index<loopMaxJet) and (jet.pt()>20)]
