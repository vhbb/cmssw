...
leptonTypeVHbb = NTupleObjectType("leptonTypeVHbb", baseObjectTypes = [ leptonType ], variables = [
    ...
    NTupleVariable("eleMVARawSpring16", lambda x : x.mvaRun2("GenSpring16") if abs(x.pdgId()) == 11  else -2, float, help="Electron MVA (General Spring16); -2 for muons"),
    NTupleVariable("eleMVAIdSpring16", lambda x : max(x.mvaIDRun2("GenSpring16","POG90"),2*x.mvaIDRun2("GenSpring16","POG80")) if abs(x.pdgId()) == 11  else -2, float, help="Electron ID WP:  0 none, 1 WP90, 2 WP80; -2 for muons"),
