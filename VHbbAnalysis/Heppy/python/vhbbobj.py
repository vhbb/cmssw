...
#    NTupleVariable("tightCharge",  lambda lepton : ( lepton.isGsfCtfScPixChargeConsistent() + lepton.isGsfScPixChargeConsistent() ) if abs(lepton.pdgId()) == 11 else 2*(lepton.innerTrack().ptError()/lepton.innerTrack().pt() < 0.2), int, help="Tight charge criteria"),
    NTupleVariable("mvaIdTrig",     lambda lepton : lepton.mvaId    if abs(lepton.pdgId()) == 11 else 1, help="EGamma POG MVA ID for triggering electrons; 1 for muons"),
    NTupleVariable("mvaIdTrigMediumResult",     lambda lepton : lepton.mediumIdResult    if abs(lepton.pdgId()) == 11 else 1, help="True if passes EGamma POG MVA ID for triggering electrons"),
