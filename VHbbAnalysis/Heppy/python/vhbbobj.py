...
leptonTypeVHbb = NTupleObjectType("leptonTypeVHbb", baseObjectTypes = [ leptonType ], variables = [
    ...
    NTupleVariable("eleMVArawSpring16GP", lambda x : getattr(x,"mvaRawSpring16GeneralPurpose",-2) if abs(x.pdgId()) == 11 else -1, help="EGamma POG MVA ID for non-triggering electrons (raw MVA value, Spring15 training); 1 for muons"),
    NTupleVariable("eleMVAIdSpring16GP", lambda x : max(x.mvaIdSpring16GeneralPurposePOG90, 2*x.mvaIdSpring16GeneralPurposePOG80) if abs(x.pdgId()) == 11 and hasattr(x,"mvaIdSpring16GeneralPurposePOG90")  else -1, int, help="EGamma POG MVA ID for non-triggering electrons (0=none, 1=WP90, 2=WP80, Spring15 training); 1 for muons"),
    NTupleVariable("eleMVArawSpring16HZZ", lambda x : getattr(x,"mvaRawSpring16HZZ",-2) if abs(x.pdgId()) == 11 else -1, help="HZZ electron MVA ID (raw MVA value, Spring16 training); 1 for muons"),
    NTupleVariable("eleMVAIdSpring16HZZ", lambda x : getattr(x,"mvaIdSpring16HZZ",-2) if abs(x.pdgId()) == 11 else -1, help="HZZ electron MVA ID (0=none, 1=HZZ, Spring16 training); 1 for muons"),
    ##NTupleVariable("eleMVArawSpring16GPheppy", lambda x : getattr(x,"mvaRawSpring16GP",-2) if abs(x.pdgId()) == 11 else -1, help="EGamma POG MVA ID for non-triggering electrons (raw MVA value, Spring16 training); 1 for muons"),
    NTupleVariable("eleMVAIdSpring16GPaddon", lambda x : (x.mvaIdSpring16GPVLoose + 2*x.mvaIdSpring16GPVLooseEmu + 4*x.mvaIdSpring16GPTight) if abs(x.pdgId()) == 11 and hasattr(x,"mvaIdSpring16GPVLoose") else -1, help="EGamma POG MVA ID for non-triggering electrons (0=none, VLoose + 2*VLooseIdEmu + 4*Tight otherwise, Spring16 training); 1 for muons"),
