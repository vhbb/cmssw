...
#    NTupleVariable("mvaIdTrig",     lambda lepton : lepton.mvaTrigV0(full5x5=True)    if abs(lepton.pdgId()) == 11 else 1, help="EGamma POG MVA ID for triggering electrons; 1 for muons"),
    NTupleVariable("eleMVAIdPhys14", lambda x : (x.mvaIDRun2("NonTrigPhys14", "Loose") + x.mvaIDRun2("NonTrigPhys14", "Tight")) if abs(x.pdgId()) == 11 else -1, int, help="Electron mva id working point: 1=loose, 2=tight"),    
    # Muon-speficic info
