...
class HTXSRivetAnalyzer : public edm::EDProducer
{
  public:
...
ROOTTYPE* 
  HTXSRivetAnalyzer::prebook(const YODATYPE* yodah, const std::string& name){
  ROOTTYPE* h = 0;
  if (yodah->axis().isFixedBinning() ) {//equidistant binning (easier case)
    ...
    if (! vax ){
      throw cms::Exception("HTXSRivetAnalyzer") << "Cannot dynamix cast an YODA axis to VariAxis ";
    }
...
template<> 
TH1F* HTXSRivetAnalyzer::yoda2root(const YODA::IHistogram1D* yodah, const std::string& name){
...
template<>
TProfile* HTXSRivetAnalyzer::yoda2root(const YODA::IProfile1D* yodah, const std::string& name){
