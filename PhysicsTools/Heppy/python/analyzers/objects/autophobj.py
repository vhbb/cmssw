...
jetType = NTupleObjectType("jet",  baseObjectTypes = [ fourVectorType ], variables = [
    ...
    NTupleVariable("mcMatchId",  lambda x : getattr(x, 'mcMatchId', -99), int, mcOnly=True, help="Match to source from hard scatter (pdgId of heaviest particle in chain, 25 for H, 6 for t, 23/24 for W/Z), zero if non-prompt or fake"),
    NTupleVariable("corr_JECUp",  lambda x : getattr(x, 'corrJECUp', -99), float, mcOnly=True, help="pt correction from JEC up"),
    NTupleVariable("corr_JECDown",  lambda x : getattr(x, 'corrJECDown', -99), float, mcOnly=True, help="pt correction from JEC down"),
    NTupleVariable("corr",  lambda x : getattr(x, 'corr', -99), float, mcOnly=True, help="pt correction from JEC"),
    NTupleVariable("corr_JERUp",  lambda x : getattr(x, 'corrJERUp', -99), float, mcOnly=True, help="pt correction from JER up"),
    NTupleVariable("corr_JERDown", lambda x : getattr(x, 'corrJERDown', -99), float, mcOnly=True, help="pt correction from JER down "),
    NTupleVariable("corr_JER", lambda x : getattr(x, 'corrJER', -99), float, mcOnly=True, help="pt correction from JER"),
])
