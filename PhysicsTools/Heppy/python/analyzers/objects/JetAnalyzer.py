...
def shiftJERfactor(JERShift, aeta):
    factor = 1.061 + JERShift*0.023
    if   aeta > 3.2: factor = 1.320 + JERShift * 0.286
    elif aeta > 3.0: factor = 1.303 + JERShift * 0.111
    elif aeta > 2.5: factor = 1.343 + JERShift * 0.123
    elif aeta > 1.9: factor = 1.126 + JERShift * 0.094
    elif aeta > 1.3: factor = 1.106 + JERShift * 0.030
    elif aeta > 0.8: factor = 1.088 + JERShift * 0.029
    return factor 
...
class JetAnalyzer( Analyzer ):
    ...
    def process(self, event):
        ...
        if self.cfg_ana.copyJetsByValue: 
          import ROOT
          self.allJets = map(lambda j:Jet(ROOT.pat.Jet(ROOT.edm.Ptr(ROOT.pat.Jet)(ROOT.edm.ProductID(),j,0))), self.handles['jets'].product()) #copy-by-value is safe if JetAnalyzer is ran more than once
        else: 
        ...
        if self.cfg_comp.isMC:
            ...
            if self.cfg_ana.do_mc_match:
                for igj, gj in enumerate(self.genJets):
                    gj.index = igj
                self.matchJets(event, self.allJets)
