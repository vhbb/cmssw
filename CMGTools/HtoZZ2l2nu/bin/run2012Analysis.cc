...
int main(int argc, char* argv[])
{
  ...
  optim_Cuts1_met.push_back(168); optim_Cuts1_mtmin.push_back(449); optim_Cuts1_mtmax.push_back(752);
   ...
   for(size_t ivar=0; ivar<nvarsToInclude; ivar++)
   {
       ...
       if(ivar==0)
	 {
	   mon.addHistogram( new TH2F (TString("met_shapes")+varNames[ivar],";cut index;E_{T}^{miss} [GeV/c^{2}];",optim_Cuts1_met.size(),0,optim_Cuts1_met.size(), 100,0,500) );
	   mon.addHistogram( new TH2F (TString("redMet_shapes")+varNames[ivar],";cut index;red E_{T}^{miss} [GeV/c^{2}];",optim_Cuts1_met.size(),0,optim_Cuts1_met.size(),100,0,500) );
	 }
       mon.addHistogram( new TH2F (TString("lepcount")+varNames[ivar],";cut index;#Leptons;",optim_Cuts1_met.size(),0,optim_Cuts1_met.size(), 2,0,2));
  ...
  //event Categorizer
  EventCategory eventCategoryInst(3); //binned analysis
	  ...
	  if(mt>optim_Cuts1_mtmin[index] && mt<optim_Cuts1_mtmax[index])
	    {
	      if(passPreselection)
		{
		  mon.fillHisto(TString("met_shapes")+varNames[ivar],tags_full,index, zvv.pt(),iweight);
		  mon.fillHisto(TString("redMet_shapes")+varNames[ivar],tags_full,index, redMet.pt(),iweight);
		  if(redMet.pt()>optim_Cuts1_met[index])  mon.fillHisto(TString("mt_redMet_shapes")+varNames[ivar],tags_full,index, mt,iweight);   
		}
	      ...
	      if(zvv.pt()>optim_Cuts1_met[index])
		{
		  ...
		  mon.fillHisto(TString("lepcount")+varNames[ivar],tags_full,index, (passPreselectionM3dlep && !pass3dLeptonVeto )  ,iweight);
